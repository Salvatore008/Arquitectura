...
int main() {
    ...
    cout<<"Autores:\n\t\tLuigi Quero, C.I: 30.009.785\n\t\tSebastian Piñango, C.I: 29.947.488"<<endl;
    cout<<"\n\nLeyendo archivo \"SGY/archivo.sgy\" "<<endl<<endl;

    fd = open("SGY/archivo.sgy", O_RDONLY);
    ...
    if (fstat(fd, &sb) == -1) {
    	cerr << "ERROR al intentar obtener información del archivo." << endl;
    	close(fd);
    	return 1;
    }
    ...
    mem = (char*) mmap(NULL, sb.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
   	if (mem == MAP_FAILED) {
    	cerr << "ERROR al intentar mapear el archivo en memoria." << endl;
    	close(fd);
    	return 1;
   	}
	...
	for (int i = 0; i < coordenadasTodasTrazas.size(); i++) {
	    cout<< i + 1;
	    cout << "\nCoordenada X: " << coordenadasTodasTrazas[i].x << "\nCoordenada Y: " << coordenadasTodasTrazas[i].y << "\nCoordenada Z: " << coordenadasTodasTrazas[i].z << endl << endl;
	}
	        
	// Liberar la memoria mapeada 
	if (munmap(mem, sb.st_size) == -1) {
	    cerr << "ERROR al liberar la memoria mapeada" << endl;
	    close(fd);
	    return 1;
	}
